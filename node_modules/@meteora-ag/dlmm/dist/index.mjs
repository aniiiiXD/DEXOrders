...
function toAmountBidSide(activeId, totalAmount, distributions, mintY, clock) {
  ...
  if (totalWeight.cmp(new Decimal2(0)) != 1) {
  ...
  return distributions.map((bin) => {
    if (bin.binId > activeId) {
      ...
      return {
        binId: bin.binId,
        amount: new BN4(
          new Decimal2(totalAmount.toString()).mul(new Decimal2(bin.weight).div(totalWeight)).floor().toString()
        )
      };
    }
  });
...
function toAmountAskSide(activeId, binStep, totalAmount, distributions, mintX, clock) {
  ...
  const totalWeight = distributions.reduce(function(sum, el) {
    if (el.binId < activeId) {
      ...
      const weightPerPrice = new Decimal2(el.weight).div(price);
  ...
  return distributions.map((bin) => {
    if (bin.binId < activeId) {
      ...
      const weightPerPrice = new Decimal2(bin.weight).div(price);
      return {
        binId: bin.binId,
        amount: new BN4(
          new Decimal2(totalAmount.toString()).mul(weightPerPrice).div(totalWeight).floor().toString()
        )
      };
    }
  });
...
function toAmountBothSide(activeId, binStep, amountX, amountY, amountXInActiveBin, amountYInActiveBin, distributions, mintX, mintY, clock) {
  ...
  if (activeBins.length === 1) {
    ...
    let wx0 = new Decimal2(0);
    let wy0 = new Decimal2(0);
    const activeBin = activeBins[0];
    if (amountXInActiveBin.isZero() && amountYInActiveBin.isZero()) {
      wx0 = new Decimal2(activeBin.weight).div(p0.mul(new Decimal2(2)));
      wy0 = new Decimal2(activeBin.weight).div(new Decimal2(2));
    } else {
      let amountXInActiveBinDec = new Decimal2(amountXInActiveBin.toString());
      let amountYInActiveBinDec = new Decimal2(amountYInActiveBin.toString());
      if (!amountXInActiveBin.isZero()) {
        wx0 = new Decimal2(activeBin.weight).div(
          p0.add(amountYInActiveBinDec.div(amountXInActiveBinDec))
        );
      }
      if (!amountYInActiveBin.isZero()) {
        wy0 = new Decimal2(activeBin.weight).div(
          new Decimal2(1).add(
    ...
    distributions.forEach((element) => {
      if (element.binId < activeId) {
        totalWeightY = totalWeightY.add(new Decimal2(element.weight));
      }
      if (element.binId > activeId) {
        let price = getPriceOfBinByBinId(element.binId, binStep);
        let weighPerPrice = new Decimal2(element.weight).div(price);
        totalWeightX = totalWeightX.add(weighPerPrice);
      }
    });
    const kx = new Decimal2(amountX.toString()).div(totalWeightX);
    const ky = new Decimal2(amountY.toString()).div(totalWeightY);
    let k = kx.lessThan(ky) ? kx : ky;
    return distributions.map((bin) => {
      if (bin.binId < activeId) {
        const amount = k.mul(new Decimal2(bin.weight));
        return {
          binId: bin.binId,
          amountX: new BN4(0),
      ...
      if (bin.binId > activeId) {
        const price = getPriceOfBinByBinId(bin.binId, binStep);
        const weighPerPrice = new Decimal2(bin.weight).div(price);
    ...
    let totalWeightX = new Decimal2(0);
    let totalWeightY = new Decimal2(0);
    distributions.forEach((element) => {
      if (element.binId < activeId) {
        totalWeightY = totalWeightY.add(new Decimal2(element.weight));
      } else {
    ...
    let kx = new Decimal2(amountX.toString()).div(totalWeightX);
    let ky = new Decimal2(amountY.toString()).div(totalWeightY);
    let k = kx.lessThan(ky) ? kx : ky;
    return distributions.map((bin) => {
      if (bin.binId < activeId) {
        const amount = k.mul(new Decimal2(bin.weight));
        ...
        let weighPerPrice = new Decimal2(bin.weight).div(price);
        const amount = k.mul(weighPerPrice);
        return {
          binId: bin.binId,
          amountX: new BN4(amount.floor().toString()),
  ...
  if (activeBins.length === 1) {
    ...
    distributions.forEach((element) => {
      ...
      if (element.binId > activeId) {
        ...
        const weighPerPrice = new Decimal2(element.weight).div(price);
        totalWeightX = totalWeightX.add(weighPerPrice);
      }
    });
    const kx = totalWeightX.isZero() ? new Decimal2(1) : new Decimal2(amountX.toString()).div(totalWeightX);
...
function autoFillXByWeight(activeId, binStep, amountY, amountXInActiveBin, amountYInActiveBin, distributions) {
  ...
  if (activeBins.length === 1) {
    ...
    const ky = totalWeightY.isZero() ? new Decimal2(1) : new Decimal2(amountY.toString()).div(totalWeightY);
...
function getPriceOfBinByBinId(binId, binStep) {
  ...
  return new Decimal3(1).add(new Decimal3(binStepNum)).pow(new Decimal3(binId));
}
...
function getC(amount, binStep, binId, baseTokenDecimal, quoteTokenDecimal, minPrice, maxPrice, k) {
  const currentPricePerLamport = new Decimal4(1 + binStep / 1e4).pow(
    binId.toNumber()
  );
  const currentPricePerToken = currentPricePerLamport.mul(
    new Decimal4(10 ** (baseTokenDecimal - quoteTokenDecimal))
  );
  const priceRange = maxPrice.sub(minPrice);
  const currentPriceDeltaFromMin = currentPricePerToken.sub(
    new Decimal4(minPrice)
  );
  const c = new Decimal4(amount.toString()).mul(
...
function findOptimumDecompressMultiplier(binAmount, tokenDecimal) {
  let multiplier = new BN6(10).pow(tokenDecimal);
  while (!multiplier.isZero()) {
    ...
    for (const [_binId, amount] of binAmount) {
      ...
      if (compressedAmount.isZero()) {
        multiplier = multiplier.div(new BN6(10));
        found = false;
        break;
      }
    }
...
function generateAmountForBinRange(amount, binStep, tokenXDecimal, tokenYDecimal, minBinId, maxBinId, k) {
  const toTokenMultiplier = new Decimal4(10 ** (tokenXDecimal - tokenYDecimal));
...
var DLMM = class {
  ...
  static getPricePerLamport(tokenXDecimal, tokenYDecimal, price) {
    return new Decimal6(price).mul(new Decimal6(10 ** (tokenYDecimal - tokenXDecimal))).toString();
  }
  static getBinIdFromPrice(price, binStep, min) {
    const binStepNum = new Decimal6(binStep).div(new Decimal6(BASIS_POINT_MAX));
    const binId = new Decimal6(price).log().dividedBy(new Decimal6(1).add(binStepNum).log());
  ...
  static calculateFeeInfo(baseFactor, binStep, baseFeePowerFactor) {
    ...
    const baseFeeRatePercentage = new Decimal6(baseFeeRate.toString()).mul(new Decimal6(100)).div(new Decimal6(FEE_PRECISION.toString()));
    const maxFeeRatePercentage = new Decimal6(MAX_FEE_RATE.toString()).mul(new Decimal6(100)).div(new Decimal6(FEE_PRECISION.toString()));
    return {
  ...
  getFeeInfo() {
    ...
    const protocolFeePercentage = new Decimal6(protocolShare.toString()).mul(new Decimal6(100)).div(new Decimal6(BASIS_POINT_MAX));
    return {
      baseFeeRatePercentage,
      maxFeeRatePercentage,
  ...
  getDynamicFee() {
    ...
    return new Decimal6(totalFee.toString()).div(new Decimal6(FEE_PRECISION.toString())).mul(100);
  }
  /**
  ...
  getEmissionRate() {
    ...
    return {
      rewardOne: rewardOneEmissionRate ? new Decimal6(rewardOneEmissionRate.toString()).div(PRECISION) : void 0,
      rewardTwo: rewardTwoEmissionRate ? new Decimal6(rewardTwoEmissionRate.toString()).div(PRECISION) : void 0
    };
  }
  /**
  ...
  fromPricePerLamport(pricePerLamport) {
    return new Decimal6(pricePerLamport).div(
      new Decimal6(
  ...
  swapQuoteExactOut(outAmount, swapForY, allowedSlippage, binArrays, maxExtraBinArrays = 0) {
    ...
    const priceImpact = startPrice.sub(endPrice).abs().div(startPrice).mul(new Decimal6(100));
  ...
  async seedLiquidity(owner, seedAmount, curvature, minPrice, maxPrice, base, payer, feeOwner, operator, lockReleasePoint, shouldSeedPositionOwner = false) {
    let tokenOwnerProveAssociatedTokenAccountLamports = new BN13(0);
    let totalPositionCount = new BN13(0);
    let totalPositionLamports = new BN13(0);
    let totalBinArraysCount = new BN13(0);
    let totalBinArraysLamports = new BN13(0);
    let binArrayBitmapLamports = new BN13(0);
    const toLamportMultiplier = new Decimal6(
      10 ** (this.tokenY.mint.decimals - this.tokenX.mint.decimals)
    );
    const minPricePerLamport = new Decimal6(minPrice).mul(toLamportMultiplier);
    const maxPricePerLamport = new Decimal6(maxPrice).mul(toLamportMultiplier);
    ...
    for (let i = 0; i < positionCount.toNumber(); i++) {
      ...
      for (let i2 = 0; i2 < binArrayAccountMetas.length; i2++) {
        ...
        if (!account && !appendedInitBinArrayIx.has(pubkey)) {
          totalBinArraysCount = totalBinArraysCount.add(new BN13(1));
      ...
      if (!positionAccount) {
        totalPositionCount = totalPositionCount.add(new BN13(1));
  ...
  static async processPosition(program, lbPair, clock, position, baseMint, quoteMint, rewardMint0, rewardMint1, binArrayMap) {
    ...
    if (!bins.length)
      return null;
    const positionData = [];
    let totalXAmount = new Decimal6(0);
    let totalYAmount = new Decimal6(0);
        ...
        if (binId >= lowerBinId && binId <= upperBinId) {
          ...
          if (!binArray) {
            bins.push({
              ...
              price: pricePerLamport,
              version: 2,
              pricePerToken: new Decimal6(pricePerLamport).mul(new Decimal6(10 ** (baseTokenDecimal - quoteTokenDecimal))).toString()
            });
          } else {
