...
var POSITION_V2_DISC = _anchor.BorshAccountsCoder.accountDiscriminator("positionV2");
...
async function getTokensMintFromPoolAddress(connection, poolAddress, opt) {
  ...
  const program = new (0, _anchor.Program)(
    IDL,
    _nullishCoalesce(opt.programId, () => ( LBCLMM_PROGRAM_IDS[_nullishCoalesce(_optionalChain([opt, 'optionalAccess', _29 => _29.cluster]), () => ( "mainnet-beta"))])),
    provider
  );
...
var DLMM = class {
  ...
  static async getAllLbPairPositionsByUser(connection, userPubKey, opt) {
    ...
    for (const position of positionWrappers) {
      ...
      const reserveXBalance = _nullishCoalesce(_optionalChain([lbPairReserveMap, 'access', _57 => _57.get, 'call', _58 => _58(lbPair.toBase58()), 'optionalAccess', _59 => _59.reserveX]), () => ( BigInt(0)));
      const reserveYBalance = _nullishCoalesce(_optionalChain([lbPairReserveMap, 'access', _60 => _60.get, 'call', _61 => _61(lbPair.toBase58()), 'optionalAccess', _62 => _62.reserveY]), () => ( BigInt(0)));
  ...
  async getPosition(positionPubKey) {
    ...
    return {
      ...
      positionData: await DLMM.processPosition(
        ...
        this.tokenX.mint,
        this.tokenY.mint,
        _optionalChain([this, 'access', _78 => _78.rewards, 'access', _79 => _79[0], 'optionalAccess', _80 => _80.mint]),
        _optionalChain([this, 'access', _81 => _81.rewards, 'access', _82 => _82[1], 'optionalAccess', _83 => _83.mint]),
        binArrayMap
      ),
      version: position.version()
    };
  ...
  async removeLiquidity({
    ...
    const { slices, accounts: transferHookAccounts } = this.getPotentialToken2022IxDataAndAccounts(0 /* Liquidity */);
    const binArrayAccountsMeta = getBinArrayAccountMetasCoverage(
      new (0, _anchor.BN)(fromBinId),
      new (0, _anchor.BN)(toBinId),
      this.pubkey,
      this.program.programId
    );
    const preInstructions = [];
    ...
    if (shouldClaimAndClose) {
      ...
      for (let i = 0; i < 2; i++) {
        ...
        const claimRewardIx = await this.program.methods.claimReward2(new (0, _anchor.BN)(i), fromBinId, toBinId, {
          slices: slices2
        }).accounts({
          lbPair: this.pubkey,
          sender: user,
    ...
    if (maxExtraBinArrays > 0 && maxExtraBinArrays <= MAX_EXTRA_BIN_ARRAYS) {
      ...
      while (extraBinArrays.length < maxExtraBinArrays) {
        let binArrayAccountToSwap = findNextBinArrayWithLiquidity(
          ...
          activeId,
          this.lbPair,
          _nullishCoalesce(_optionalChain([this, 'access', _90 => _90.binArrayBitmapExtension, 'optionalAccess', _91 => _91.account]), () => ( null)),
          binArrays
        );
        if (binArrayAccountToSwap == null) {
          break;
  ...
  canSyncWithMarketPrice(marketPrice, activeBinId) {
    ...
    const toBinArrayIndex = findNextBinArrayIndexWithLiquidity(
      ...
      this.lbPair,
      _nullishCoalesce(_optionalChain([this, 'access', _92 => _92.binArrayBitmapExtension, 'optionalAccess', _93 => _93.account]), () => ( null))
    );
